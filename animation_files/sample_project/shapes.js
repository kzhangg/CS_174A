...
function windmill( num_blades )
{
shape.call(this); // Inherit class shape’s array members by calling parent constructor
this.populate( num_blades ); // A new windmill immediately populates its own arrays with windmill points,
this.init_buffers(); // Then sends its arrays to the graphics card into new buffers
}
inherit(windmill, shape);
windmill.prototype.populate = function( num_blades )
{
 this.vertices.push( vec3( 0, 0, 0 ) ); // Center vertex - shared via index 0 to all triangles
 this.normals. push( vec3( 0, -1, 0 ) ); // (Not planning on flat shading)
 this.texture_coords.push( vec2( 0, 0 ) );
 for( var i = 0; i < num_blades; i++ ) // The rest of the vertices.
 {
 var spin = rotation( i * 360/num_blades, 0, 1, 0 );
 var newPoint = mult_vec( spin, vec4( 1, 0, 0, 1 ) );
 this.vertices.push( vec3( newPoint[0], 0, newPoint[2] ) );
 this.vertices.push( vec3( newPoint[0], 1, newPoint[2] ) );
var newNormal = mult_vec( spin, vec3( 0, 0, 1 ) );
 this.normals.push( vec3( newNormal ) );
 this.normals.push( vec3( newNormal ) );
...
// Class tetrahedron - demo of flat vs smooth shading
function tetrahedron()
{
 shape.call(this); // Inherit class shape’s array members by calling parent constructor
 this.populate(); // It immediately populates its arrays
 this.init_buffers(); // Then sends its arrays to the graphics card into new buffers
}
...
// Method 1: A tetrahedron with shared vertices. Compact, performs better, but can't have seams (flat shading / textures)
...
function trashcan(points_transform) {
    shape.call(this);
    if (!arguments.length) return;
    this.populate(this, points_transform);
    this.init_buffers();
}
inherit(trashcan, shape);

trashcan.prototype.populate = function (recipient, points_transform) {
    var offset = recipient.vertices.length;
    var index_offset = recipient.indices.length;

    //front trapezoid vertices
    var a = vec3(-2, 1.5, 1);
    var b = vec3(2, 1.5, 1);
    var c = vec3(-1, -1.5, 1);
    var d = vec3(1, -1.5, 1);

    //back trapezoid vertices
    var e = vec3(-2, 1.5, -1);
    var f = vec3(2, 1.5, -1);
    var g = vec3(-1, -1.5, -1);
    var h = vec3(1, -1.5, -1);
    ...
    recipient.vertices.push(a, b, c);
    recipient.vertices.push(c, b, d);
    // back trapezoid face
    recipient.vertices.push(e, f, g);
    recipient.vertices.push(g, f, h);

    // Bottom side
    recipient.vertices.push(c, d, g);
    recipient.vertices.push(g, d, h);
    // Left side
    recipient.vertices.push(e, a, g);
    recipient.vertices.push(g, a, c);
    // Right side
    recipient.vertices.push(f, b, h);
    recipient.vertices.push(h, b, d);
